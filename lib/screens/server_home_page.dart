...
class _ServerHomePageState extends State<ServerHomePage> {
  ...
  Future<void> _initializeDirectory() async {
    try {
      ...
      // Use app's external media directory instead of data directory
      if (Platform.isAndroid) {
        // Get the external storage directory and convert from data to media path
        final externalDir = await getExternalStorageDirectory();
        if (externalDir != null) {
          // Convert from /Android/data/package/ to /Android/media/package/
          final mediaPath = externalDir.path.replaceFirst('/Android/data/', '/Android/media/');
          directory = Directory(path.join(mediaPath, 'shared_files'));
        }
